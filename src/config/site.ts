...
export const siteConfig = {
  ...
  status: {
    ...
    zh: "HUMANLAYA AI 创始工程师",
  } as L,
  description: {
    en: "AI algorithm & infra engineer and project manager: focused on building high-quality training and evaluation data, designing data-synthesis algorithms around an Expert-in-the-Loop philosophy, and building enterprise AI infrastructure — evaluation, data-synthesis, and data-delivery platforms. Off the clock: photography, cycling, and trekking.",
    zh: "AI 算法 & Infra 工程师 & 项目经理：主攻高质量训练与评测数据构建，以 Expert-in-Loop 为理念设计数据合成算法，同时负责企业 AI Infra 设施搭建，包括评测平台、数据合成平台、数据交付平台等；闲暇时间热爱摄影、骑行与徒步。",
  } as L,
  bio: {
    en: "AI algorithm engineer working on evaluation science, synthetic/RL data, and agentic systems, with the infrastructure background to run them at scale. Founding engineer @ HUMANLAYA AI; previously @ Fintopia, ByteDance (Doubao VLM evals), and PwC.",
    zh: "AI 算法工程师，方向为评测科学、合成/RL 数据与智能体系统，也有把它们跑在生产规模上的基础设施功底。现任 HUMANLAYA AI 创始工程师；此前在 Fintopia、字节跳动（豆包 VLM 评测）与普华永道。",
  } as L,
